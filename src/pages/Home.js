      ...
      <section className="blog-section">
        ...
        {loading ? (
          <div className="blog-grid">
            <p style={{ textAlign: 'center', color: '#666' }}>Loading blogs...</p>
          </div>
        ) : blogPosts.length > 0 ? (
          <div className="blog-grid">
            {blogPosts.map((blog) => {
              // Get banner URL - handle both Cloudinary URLs and local paths
              const bannerUrl = blog.banner && (blog.banner.startsWith('http://') || blog.banner.startsWith('https://'))
                ? blog.banner
                : blog.url && (blog.url.startsWith('http://') || blog.url.startsWith('https://'))
                  ? blog.url
                  : blog.banner
                    ? `${API_BASE.replace('/api', '')}/blogs/${blog.blogid || blog.id}/${blog.banner}`
                    : blogs;
                ...
                >
                  <div className="blog-image">
                    <img
                      ...
                      onError={(e) => { e.target.src = blogs; }}
                    />
                  </div>
                  <h3 className="blog-card-title">{blog.title || blog.name}</h3>
                  <p className="blog-date">{formatDate(blog.publishdate || blog.createdAt || blog.date)}</p>
                </div>
              );
            })}
          </div>
          ...
          <div className="blog-grid">
            <p style={{ textAlign: 'center', color: '#666' }}>No blogs available</p>
          </div>
        )}
        <button className="read-more-btn" onClick={() => navigate('/blogs')}>Read more articles</button>
      </section>
